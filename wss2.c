 ...
 * long. Instead, I copy (snapshot) the idle bitmap and pagemap into our memory 
 * with the fewest syscalls allowed, and then process them with load/stores. 
 * Much faster, at the cost of some memory.
 */
...
int loadidlemap()
{
	...
	p = g_idlebuf;
	// unfortunately, larger reads aren't supported
